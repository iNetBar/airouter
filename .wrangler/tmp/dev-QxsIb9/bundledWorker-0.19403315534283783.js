...
// .wrangler/tmp/bundle-z5t8eD/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/pages-hIxE3g/bundledWorker-0.19403315534283783.mjs
      ...
      if (handler) {
        try {
          ...
          if (err2 instanceof Error && onError) {
            context22.error = err2;
            res = await onError(err2, context22);
            isError = true;
          } else {
            throw err2;
          }
...
var handleParsingNestedValues = /* @__PURE__ */ __name2((form, key, value, state) => {
...
var throwNestingLimitExceeded = /* @__PURE__ */ __name2(() => {
...
var tryDecodeURIComponent = /* @__PURE__ */ __name2((str) => str.indexOf("%") !== -1 ? tryDecode(str, decodeURIComponent_) : str, "tryDecodeURIComponent");
    ...
    if (headers) {
      if (!responseHeaders) {
        ...
        for (const k in headers) {
          if (++count32 > 1 || typeof headers[k] !== "string") {
            responseHeaders = new Headers();
            break;
          }
        }
...
var errorHandler = /* @__PURE__ */ __name2((err2, c) => {
...
var createNullObject = /* @__PURE__ */ __name2(() => /* @__PURE__ */ Object.create(null), "createNullObject");
var emptyParam = [];
...
var Node = /* @__PURE__ */ __name2(/* @__PURE__ */ __name(class _Node {
  ...
  insert(tokens, index, paramMap, context22, isStatic) {
    let node = this;
    for (let i = 0, len = tokens.length; i < len; i++) {
      ...
      if (pattern) {
        ...
        if (name !== "") {
          nextNode.#varIndex ??= context22.varIndex++;
...
__name2(getProvider, "getProvider");
...
__name2(saveProvider, "saveProvider");
...
__name2(deleteProvider, "deleteProvider");
...
var TRIM_KEEP = 2e3;
var LogRing = /* @__PURE__ */ __name2(class {
  buf = [];
  timer = null;
  push(l) {
    this.buf.push(l);
...
__name2(migrateBuiltins, "migrateBuiltins");
...
__name2(maskToken, "maskToken");
async function isAdmin(req, env22) {
  ...
  try {
    ...
    const expected = await hmacSha256(env22.SESSION_SECRET, "admin");
...
__name2(patternToRegex, "patternToRegex");
...
var worker_default = {
  async fetch(request, env22, ctx) {
    ...
    app.use("*", logger());
    if (!builtinsInit) {
      builtinsInit = migrateBuiltins(env22.DB).catch((e) => console.error("migrateBuiltins failed:", e));
    }
    ...
    app.post("/v1/chat/completions", async (c) => {
      ...
      const expectedKey = env22.PROXY_KEY || (await getSettingsCached(env22)).apiToken;
      if (!expectedKey)
      ...
      const start = Date.now();
      const routes = await getRoutes(env22.DB);
      ...
      const providers = await getProviders(env22.DB);
      const tryList = matched ? [...matched.providers, ...matched.fallback] : providers.filter((p) => p.enabled).map((p) => p.id);
      let lastErr = null;
      for (const pid22 of tryList) {
        const p = providers.find((x) => x.id === pid22);
        if (!p)
          continue;
        ...
        const keys = await getKeys(env22.DB, p.id);
        if (keys.length === 0)
          continue;
        const key = keys[Math.floor(Math.random() * keys.length)];
        let apiKey;
        try {
          apiKey = await decrypt(key.secret, env22.ENCRYPT_KEY);
        } catch (e) {
    ...
    app.post("/admin/api/login", async (c) => {
      ...
      if (typeof password !== "string" || !timingSafeEqual(password, env22.DEFAULT_ADMIN_PASS)) {
      ...
      const sig = await hmacSha256(env22.SESSION_SECRET, "admin");
    ...
    app.get("/admin/api/dashboard", async (c) => {
      if (!await isAdmin(c.req.raw, env22))
        return err(401, "Unauthorized");
      const [providers, routes, keys, stats, recent, settings] = await Promise.all([
        getProviders(env22.DB),
        getRoutes(env22.DB),
        getKeys(env22.DB),
        getStats(env22.DB),
        recentLogs(env22.DB, 10),
        getSettings(env22.DB)
      ]);
      const modelCount = /* @__PURE__ */ new Map();
      for (const l of recent)
        modelCount.set(l.model, (modelCount.get(l.model) || 0) + 1);
      const modelRanking = [...modelCount.entries()].map(([model, count32]) => ({ model, count: count32 })).sort((a, b) => b.count - a.count).slice(0, 10);
    ...
    app.get("/admin/api/settings", async (c) => {
      ...
      const s = await getSettings(env22.DB);
      return c.json({
        projectName: s.projectName || env22.PROJECT_NAME || "iRouter",
        baseUrl: s.baseUrl || env22.BASE_URL || "",
        apiToken: s.tokenMasked || maskToken(env22.API_TOKEN || "")
      });
    });
    app.put("/admin/api/settings", async (c) => {
      ...
      const cur = await getSettings(env22.DB);
      const next = { ...cur };
      ...
      await saveSettings(env22.DB, next);
    ...
    app.get("/admin/api/providers", async (c) => {
      if (!await isAdmin(c.req.raw, env22))
        return err(401, "Unauthorized");
      return c.json(await getProviders(env22.DB));
    });
    ...
    app.put("/admin/api/providers/:id", async (c) => {
      ...
      const cur = await getProvider(env22.DB, id);
      if (!cur)
      ...
      await saveProvider(env22.DB, next);
      return c.json({ ok: true });
    });
    app.delete("/admin/api/providers/:id", async (c) => {
      if (!await isAdmin(c.req.raw, env22))
        return err(401, "Unauthorized");
      await deleteProvider(env22.DB, c.req.param("id"));
      return c.json({ ok: true });
    });
    app.get("/admin/api/routes", async (c) => {
      if (!await isAdmin(c.req.raw, env22))
        return err(401, "Unauthorized");
      return c.json(await getRoutes(env22.DB));
    });
...
__name2(getKeyMaterial, "getKeyMaterial");
async function encrypt(text, key) {
  ...
  const b64 = /* @__PURE__ */ __name2((buf) => btoa(String.fromCharCode(...buf)), "b64");
...
async function decrypt(payload, key) {
  ...
  const fromB64 = /* @__PURE__ */ __name2((s) => Uint8Array.from(atob(s), (c) => c.charCodeAt(0)), "fromB64");
  const tryDecrypt = /* @__PURE__ */ __name2(async (k) => {
...
// .wrangler/tmp/bundle-z5t8eD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-z5t8eD/middleware-loader.entry.ts
...
//# sourceMappingURL=bundledWorker-0.19403315534283783.js.map
